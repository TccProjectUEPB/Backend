...
class OrientacaoRepository:
    ...
    async def create(self, data: Dict[str, Any], commit=True):
        insert_stmt = (
            Orientacao.__table__.insert()
            .returning(
                Orientacao.id,
                Orientacao.aluno_id,
                Orientacao.professor_id,
                Orientacao.status,
                Orientacao.title,
                Orientacao.description,
                Orientacao.metodology,
                Orientacao.created_at,
                Orientacao.updated_at,
            )
            .values(**data)
        )
    ...
    async def get_all(self, filters={}):
        ...
        return loads(
            OrientacaoList(
                root=[orientacao async for orientacao in stream]
            ).model_dump_json()
        )
    ...
    async def has_active(self, aluno_id, professor_id) -> bool:
        stmt = (
            ...
            .filter(
                ...
                Orientacao.status != OrientationType.FINALIZADO.value,
            )
            .limit(1)
        )
        stream = await self.session.stream_scalars(stmt.order_by(Orientacao.id))
