...
async def get_orientacoes_by_prof(request, professor_id):
    request.args["professor_id"] = [professor_id]
    handler = OrientacaoController().get_all
    return await sanic_request_handler(handler, request)


@ORIENTACAO_ROUTES.route("/professores/<professor_id:str>/orientacoes/<orientacao_id:str>", methods=["GET"])
@authenticated("orp:r")
@admin_or_id("professor_id")
async def get_orientacao_by_prof(request, professor_id: str, orientacao_id: str):
    handler = partial(OrientacaoController().get_one_by_prof, professor_id, orientacao_id)
    return await sanic_request_handler(handler, request)
...
async def get_orientacoes_by_aluno(request, aluno_id):
    request.args["aluno_id"] = [aluno_id]
    handler = OrientacaoController().get_all
    return await sanic_request_handler(handler, request)


@ORIENTACAO_ROUTES.route("/alunos/<aluno_id:str>/orientacoes/<orientacao_id:str>", methods=["GET"])
@authenticated("ora:r")
@admin_or_id("aluno_id")
