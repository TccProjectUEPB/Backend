...
from pydantic import (
    ...
    Field, field_serializer, validator,
    EmailStr, StrictStr
)
...
class CreateAlunoModel(CreateAuthModel):
...
class AlunoQueryModel(QueryModel):
    ...
    def Opvalidator(cls, v):
        return [
            TypeOpStr(index) if TypeOpStr.validate_format(index) else cls.integrate_regex(index) for index in v
        ]
'''
    @validator("charge_level")
    def FloatOpvalidator(cls, v):
        lista = []
        for index in v:
            if re.match("([+-]?([0-9]*[.])?[0-9]+)$", index):
                lista.append(float(index))
            elif TypeOpFloat.validate_format(index):
                lista.append(TypeOpFloat(index))
            else:
